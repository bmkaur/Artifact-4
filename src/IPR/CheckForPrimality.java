/**
 * @class     CheckForPrimality
 * @version   0.01
 * @date      1 November 2010
 * @author    Jalal Noureddine, Sergey Khechumov, Team B
*/

package IPR;

public class CheckForPrimality {
    public boolean IsPrime(int iNum)
    {
        ...
        else
        {
            ...
            dNumSqrt = Math.sqrt(iNum)/*LocalRootN.GetRootN()*/;// Assign calculated sqrt to the dNumSqrt

            // MAIN CYCLE
            // m is a number of form 6*k-1 and 6*k+1 where k is a positive integer number
            for (int m = 5; m <= dNumSqrt; m += 2)
            {
                if (iNum % m == 0)// If m is a factor of iNum, iNum is not prime
                {
